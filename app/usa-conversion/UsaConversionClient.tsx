...
interface UsaConversionClientProps {
  realizacje: Realizacja[];
}

export default function UsaConversionClient({ realizacje }: UsaConversionClientProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
      {usaConversionRealizacje.length > 0 ? (
        usaConversionRealizacje.map((realizacja, index) => (
        ...
        <div className="col-span-full text-center py-12">
          <p className="text-gray-400 text-base sm:text-lg">
            Wkrótce pojawią się tutaj realizacje związane z konwersją USA
          </p>
        </div>
      )}
    </div>
